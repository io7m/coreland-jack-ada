...
package jack.transport is
  ...
  -- unique ID
  type unique_t is new types.uint64_t;

  -- position bits
  type position_bits_t is new types.unsigned_t;
  ...
  type padding_t is array (1 .. 7) of types.int32_t;
  ...
  type position_t is record
    unique_1                     : unique_t;
    usecs                        : types.time_t;
    frame_rate                   : types.num_frames_t;
    frame                        : types.num_frames_t;
    valid                        : position_bits_t;
    bar                          : types.int32_t;
    beat                         : types.int32_t;
    tick                         : types.int32_t;
    bar_start_tick               : types.double_t;
    beats_per_bar                : types.float_t;
    beat_type                    : types.float_t;
    ticks_per_beat               : types.double_t;
    beats_per_minute             : types.double_t;    
    frame_time                   : types.double_t;
    next_time                    : types.double_t;
    bbt_offset                   : types.num_frames_t;
    audio_frames_per_video_frame : types.float_t;
    video_offset                 : types.num_frames_t;
    padding                      : padding_t;
    unique_2                     : unique_t;
  end record;
  ...
  type timebase_callback_t is access procedure
    (state        : state_t;
     ...
     position     : position_access_t;
     new_position : types.integer_t;
     user_data    : types.user_data_access_t);
  pragma convention (c, timebase_callback_t);

  -- API

  function release_timebase (client : types.client_t) return types.integer_t;
  ...
  function locate
    (client : types.client_t;
     frame  : types.num_frames_t) return types.integer_t;
  ...
  function get_current_transport_frame (client : types.client_t) return types.num_frames_t;
  pragma import (c, get_current_transport_frame, "jack_get_current_transport_frame");

  function reposition
    (client   : types.client_t;
     position : position_access_t) return types.integer_t;
  pragma import (c, reposition, "jack_transport_reposition");

  procedure start (client : types.client_t);
  pragma import (c, start, "jack_transport_start");

  procedure stop (client : types.client_t);
